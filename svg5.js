...
const circle = (cx, cy, diameter) => {
    svg5.svg.innerHTML += `<circle stroke="${svg5.strokeColor}" fill="${svg5.fillColor}" cx="${cx}" cy="${cy}" r="${diameter/2}" />`
}
const ellipse = (cx, cy, w, h) => {
    svg5.svg.innerHTML += `<ellipse stroke="${svg5.strokeColor}" fill="${svg5.fillColor}" cx="${cx}" cy="${cy}" rx="${w/2}" ry="${h/2}" />`
}
const rect = (x, y, w, h) => {
    svg5.svg.innerHTML += `<rect stroke="${svg5.strokeColor}" fill="${svg5.fillColor}" x="${x}" y="${y}" width="${w}" height="${h}" />`
}
const line = (x1, y1, x2, y2) => {
    svg5.svg.innerHTML += `<path stroke="${svg5.strokeColor}" fill="${svg5.fillColor}" d="M${x1},${y1} L${x2},${y2}" />`
}
const polygon = (cx, cy, nbPoints, radius, angle=0) => {
    ...
    svg5.svg.innerHTML += `<polygon stroke="${svg5.strokeColor}" fill="${svg5.fillColor}" points="${pts.join(' ')}" />`
}
const shape = (...pts) => {
    svg5.svg.innerHTML += `<polygon stroke="${svg5.strokeColor}" fill="${svg5.fillColor}" points="${pts.reduce((acc, d, i) => acc += d + (i % 2 == 0 ? ',' : ' '), '')}" />`
}
const polyline = (...pts) => {
    svg5.svg.innerHTML += `<polyline stroke="${svg5.strokeColor}" fill="${svg5.fillColor}" points="${pts.reduce((acc, d, i) => acc += d + (i % 2 == 0 ? ',' : ' '), '')}" />`
}
const beginShape = () => {
    svg5.path = []
}
const vertex = (x, y) => {
    svg5.path.push(`${svg5.path.length == 0 ? 'M' : 'L'}${x},${y}`)
}
const bezierVertex = (x1, y1, x2, y2, x, y) => {
    svg5.path.push(`C ${x1} ${y1}, ${x2} ${y2}, ${x} ${y}`)
}
const cubicVertex = (x2, y2, x, y) => {
    svg5.path.push(`S ${x2} ${y2}, ${x} ${y}`)
}
const quadraticVertex = (x1, y1, x, y) => {
    svg5.path.push(`Q ${x1} ${y1}, ${x} ${y}`)
}
const endShape = (closed) => {
    if(closed) {
        const first = svg5.path[0]
        first[0] = 'L'
        svg5.path.push(first)
    }
    svg5.svg.innerHTML += `<path stroke="${svg5.strokeColor}" fill="${svg5.fillColor}" d="${svg5.path.join(' ')}" />`
}
